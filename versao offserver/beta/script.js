...
function atualizarUI() {
    ...
    if (estado.fase === 'LOBBY') {
        ...
        if (estado.subFase === 'PERGUNTA') {
            statusTit.innerText = `Vez de ${jogadorAtual.nome} ler!`;
            btnP.style.display = (meuId === jogadorAtual.id) ? "inline-block" : "none";
            btnR.style.display = "none";
        } else {
            statusTit.innerText = `Vez de ${jogadorProx.nome} responder!`;
            btnR.style.display = (meuId === jogadorProx.id) ? "inline-block" : "none";
...
function renderCores(jogadoresNaSala) {
    ...
    div.innerHTML = "";
    const usadas = jogadoresNaSala.map(j => j.corNome);
    coresDisponiveis.forEach(c => {
        const ocupada = usadas.includes(c.nome);
        div.innerHTML += `<div class="cor ${ocupada ? 'indisponivel' : ''} ${corSelecionada === c.nome ? 'selecionada' : ''}" 
            style="background:${c.cor}" onclick="${ocupada ? '' : `selecionarCor('${c.nome}')`}"></div>`;
    });
}
