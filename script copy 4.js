...
function renderCores() {
  ...
  coresDisponiveis.forEach(c => {
    // Verifica se a cor já foi usada por algum jogador logado no Firestore
...
function renderRoda() {
  ...
  // Função auxiliar para renderizar um jogador
...
async function addJogador() {
  ...
  await addDoc(collection(db, "jogadores"), {
    ...
    cor: corObj.cor,
    corNome: corObj.nome,
    avatarURL: '', // Placeholder, Storage virá depois
    timestamp: Date.now()
  });
...
async function addPergunta() {
  // Lógica de quem é o autor e respondente virá aqui no futuro
...
onSnapshot(query(collection(db, "jogadores"), orderBy("timestamp")), (snapshot) => {
  // Mapeia os documentos do Firestore para o array local 'jogadores'
  jogadores = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  renderRoda(); // Redesenha a roda para todo mundo com o layout bonitinho
});

onSnapshot(query(collection(db, "perguntas"), orderBy("timestamp", "asc")), (snapshot) => {
  // Isso apenas demonstra o feed, a lógica real de autor/respondente será mais complexa
  let feed = document.getElementById("feed");
  feed.innerHTML = "<h2>Feed do Jogo</h2>";
  snapshot.forEach(doc => {
    const p = doc.data();
    // Dados temporários para demonstração do layout do feed
