...
function comandoLimparServidor() {
    if (!confirm("Isso vai expulsar todos e resetar o banco de dados. Tem certeza?")) return;

    console.log("Iniciando limpeza do servidor...");

    // Estrutura limpa e padrão para o Firestore
    const estadoInicial = {
        fase: 'LOBBY',           // Volta para o início
        subFase: 'PERGUNTA',    // Reseta a subfase de leitura
        vezIndice: 0,           // Reseta o turno para o primeiro jogador
        jogadores: [],          // Remove todos os jogadores
        espectadores: [],       // Remove todos os espectadores
        posts: [],              // Limpa o histórico de mensagens/feed
        dataReset: new Date().toISOString() // Marca quando foi feito o reset
    };

    db.collection("games").doc(ID_SALA).set(estadoInicial)
    .then(() => {
        console.log("Servidor reconfigurado com sucesso!");
        
        // Limpa o seu login local para evitar conflitos de ID
        localStorage.removeItem("meuId");
        
        alert("O servidor foi limpo e reconfigurado! A página será reiniciada.");
        window.location.reload(); // Recarrega a página para limpar variáveis locais
    })
    .catch((error) => {
        console.error("Erro ao resetar servidor: ", error);
        alert("Erro ao limpar servidor. Verifique o console.");
    });
}
...
function atualizarUI() {
    ...
    if (!euPlayer && !euSpec) {
        ...
        const btnPlayer = document.getElementById("btn-entrar");
