...
function renderRoda(jogadores) {
  ...
  let baixo = jogadores.slice(metade); // Pegamos o restante sem inverter ainda

  // --- Linha de Cima (Seta DEPOIS) ---
  cima.forEach((j, i) => {
    let setaDepois = "";
    if (i < cima.length - 1) {
      setaDepois = '<div class="seta">➡️</div>';
    } else if (baixo.length > 0) {
      setaDepois = '<div class="seta">⬇️</div>';
    }
  ...
  // --- Linha de Baixo (Seta ANTES) ---
  // Invertemos apenas para a exibição visual (Direita para Esquerda)
  let baixoVisual = [...baixo].reverse(); 

  baixoVisual.forEach((j, i) => {
    let setaAntes = "";
    
    // O primeiro jogador visual da linha de baixo (na direita) 
    // NÃO tem seta antes, pois ele recebe a seta ⬇️ de cima.
    // O último jogador visual da linha de baixo (na esquerda) 
    // recebe a seta ⬆️ ANTES dele.
    
    if (i === baixoVisual.length - baixo.length) {
        setaAntes = '<div class="seta">⬆️</div>';
    } else if (i >= 0) {
        setaAntes = '<div class="seta">⬅️</div>';
    }
